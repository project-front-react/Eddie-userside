...
import { getAllEvent, getNews } from "../../services/eddiServices";
...
const EventList = () => {
  ...
  useEffect(() => {
    setIsLoader(true);
    getAllEvent().then(async (result) => {
      setIsLoader(false);
      if (result.status == "success") {
        let featuredEvent = [];
        let countResult = [];
        await result?.all_event?.map((data, i) => {
          if (
        ...
        let allEvents = result?.data?.concat(countResult);
          ...
          <div className="container">
            <div className="row">
              <div className="brdcumb-block">
                <div>
                  <Link className="brd-link" to="/user-dashboard">
                    {" "}
                    {t("Dashboard", lan)}{" "}
                  </Link>
                  |
              ...
              {featuredEvent.length > 0 && (
                ...
                >
                  ...
                  <div className="col-md-12 col-12 mx-auto text-center banner-content">
                    ...
                    <Link
                      to={`/user-dashboard/event-details/${featuredEvent[0]?.uuid}`}
